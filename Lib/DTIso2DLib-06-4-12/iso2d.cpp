...
//				&((m_pNodes)[i + INIT_NOD_NUM].pt)[1], &(m_pNodes[i + INIT_NOD_NUM].spacing));
//	#else
...
bool DTIso2D::calcDens()
{
	...
	for (iNod = INIT_NOD_NUM; iNod < m_nNodes; iNod++)
	{
		...
		for (j=0; j<m_nTriSrcNum; j++)
		{
			...
			m_triSources[j].points[2],m_pNodes[iNod].pt);
			m_pNodes[iNod].spacing = ::Min(m_pNodes[iNod].spacing, spac);
		}
	#endif
	}
...
//	for (iNod = INIT_NOD_NUM; iNod < m_nNodes; iNod++)
//	{
//		m_pNodes[iNod].spacing = Min(spacFrmBkGrnd(m_pNodes[iNod].pt), 
//			m_pNodes[iNod].spacing);
//	} // TEMP [2/18/2006]
#endif
	return true;
}
...
//	recDistPnts(); // recover boundary first [4/10/2006]
	return 1;
}
...
#ifdef _3D_DECOM_
REAL squaDist_3D(POINT3D p1, POINT3D p2)
{
	REAL s = 0.0;
	int i;
	for (i = 0; i < 3; i++)
...
int DTIso2D::writeSpa(char *fname)
{
	...
	for (i=0; i<m_nNodes; i++)
	{
		out<<i+1<<"\t"<<m_pNodes[i].spacing<<endl;
	}
	out.close();
	return 0;
}
...
//	if (m_nNodes + m_nElems > m_nAllocNodes)
//	{
//		do {
//			m_nAllocNodes += m_nAllocNodes*NUM_ADD_FAC;
//			m_pNodes = (Node *)realloc(m_pNodes, sizeof(Node)*m_nAllocNodes);
...
//				exit(1);
//			}
//		} while(m_nAllocNodes < m_nNodes+m_nElems);
//	} //  [3/28/2006]
	for (iEle=0; iEle<m_nElems; iEle++)
	{
...
#ifdef _VERBOS
	cout<<"inner point insertion after Node "<<m_nNodes<<". Current elements num: "<<m_nElems<<endl;
#endif
	if (crtInnerPnts() == 0)
		return 0;
...
//				{
//					//Not found
//					m_pNodes[pnt->iNod].iReserved = -1; //this means that node will be deleted
//					delete pnt;
//					it_old = it;
//					it++;
//					m_lstInstInnerNods.erase(it_old);
//					m_nInners--;
//				}
			}
			continue;
		...
		if (nResult == 1)
		{
			...
			//Follow code is commented as this strategy is too slow when the list is 
			//long. Now, we just check whether the corresponding element is valid right
			//before inserting the inner point. //  [2/13/2006]
//			//check later points, make sure their corresponding element is valid
//			for (it = m_lstInstInnerNods.begin(); it!=m_lstInstInnerNods.end(); )
//			{
//				InnerPnt *pnt = *it;
//				INTEGER iElem = pnt->iEle;
//				INTEGER iNode = pnt->iNod;
//				if (isDelEle(iElem))
//				{
//					//discard current node and replace with the last node
//					it_last = m_lstInstInnerNods.end();
...
//						if (it != it_last)
//						{
//							m_pNodes[iNode] = m_pNodes[pLastPnt->iNod];
//							pnt->iEle = pLastPnt->iEle;
//						}
//						else
//						{
//							it++;
...
//#ifdef _ERROR_CHK
//			char middle[20];
//			cout<<"Node "<<iNod<<": Succeed"<<"\tElems: "<<m_nElems<<endl;
//			sprintf(middle,"Mid%d.dt2",iNod);
//			writeDt2(middle);
//#endif
		}
...
//#ifdef _VERBOS
//			cout<<"Node "<<iNod<<": Rejected"<<"\tElems: "<<m_nElems<<endl;
//#endif
			it_last = m_lstInstInnerNods.end();
			it_last--;
			for (; ; it_last--)
		...
		{
//#ifdef _VERBOS
//			cout<<"Node "<<iNod<<": Failed"<<"\tElems: "<<m_nElems<<endl;
//#endif
			iFail++;
			if (iFail >= m_lstInstInnerNods.size())
			{
//#ifdef _VERBOS
//				cout<<"Node "<<iNod<<": Disturbed"<<"\tElems: "<<m_nElems<<endl;
//#endif
				if (!isDisturbed(iNod))
				{
					for (i=0; i<DIM; i++)
...
REAL DTIso2D::spacFrmPnt(PointSource src, POINT pt)
{
	...
	dist = sqrt(squaDist_3D(src.pt, pt2));
#else
...
REAL DTIso2D::spacFrmLne(PointSource src1, PointSource src2, POINT pt)
{
	...
	REAL dist;
#ifdef _3D_DECOM_
	dist = sqrt(squaDist_3D(src1.pt, src2.pt));
#else
...
//#endif
	...
	for (iBnd=0; iBnd<m_nBnds; iBnd++)
	{
		m_pBnds[iBnd].beg = pnBeg[iBnd]+INIT_NOD_NUM;
		m_pBnds[iBnd].end = pnEnd[iBnd]+INIT_NOD_NUM;
